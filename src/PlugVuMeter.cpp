...
void PlugVuMeter::paint(juce::Graphics &g) {
  ...
  const auto paperLo =
      (m_isLit ? paperLit : paperUnlit).darker(m_isLit ? 0.11f : 0.24f);

  juce::ColourGradient paperGrad(
      paperHi, juce::Point<float>(faceBounds.getCentreX(), faceBounds.getY()),
      paperLo,
  ...
  if (m_isLit) {
    juce::ColourGradient faceLift(
        ...
        juce::Point<float>(faceBounds.getX() + 18.0f,
                           faceBounds.getY() + 10.0f),
        juce::Colours::transparentWhite,
        juce::Point<float>(faceBounds.getCentreX(),
                           faceBounds.getCentreY() + 10.0f),
        false);
    ...
    juce::ColourGradient hotspot(
        ...
        juce::Colours::transparentWhite,
        juce::Point<float>(faceBounds.getCentreX(), faceBounds.getY() + 62.0f),
        true);
  ...
  const auto centre =
      juce::Point<float>(std::floor(faceBounds.getCentreX()) + 0.5f,
                         std::floor(faceBounds.getBottom() - 11.0f) + 0.5f);

  const float radius = juce::jmin(faceBounds.getWidth() * 0.47f,
                                  faceBounds.getHeight() * 0.90f) -
                       4.0f;

  const std::array<float, 11> majorTicks = {-20.0f, -10.0f, -7.0f, -5.0f,
                                            -3.0f,  -2.0f,  -1.0f, 0.0f,
                                            1.0f,   2.0f,   3.0f};

  const std::array<float, 7> labelTicks = {-20.0f, -10.0f, -7.0f, -5.0f,
                                           0.0f,   2.0f,   3.0f};
  ...
  for (float vu = minVu; vu <= maxVu + 0.5f; vu += 1.0f) {
    ...
    juce::Colour tickCol = (vu > 0.1f)
                               ? juce::Colour(0xffb53a32)
                               : juce::Colour(0xff121212).withAlpha(0.92f);
    if (!m_isLit)
    ...
    juce::Colour textCol = (vu > 0.1f)
                               ? juce::Colour(0xffa4322c)
                               : juce::Colour(0xff121212).withAlpha(0.95f);
    if (!m_isLit)
      textCol = textCol.withSaturation(0.10f).withAlpha(0.46f);

    g.setColour(textCol);
    g.drawText(labelText, static_cast<int>(labelPos.x - 8),
               static_cast<int>(labelPos.y - 4), 16, 9,
  ...
  g.drawText("VU", static_cast<int>(faceBounds.getX()),
             static_cast<int>(centre.y - 38.0f),
             static_cast<int>(faceBounds.getWidth()), 14,
             juce::Justification::centred);
  ...
  g.drawText(m_label, static_cast<int>(faceBounds.getX()),
             static_cast<int>(centre.y - 24.0f),
             static_cast<int>(faceBounds.getWidth()), 12,
             juce::Justification::centred);
