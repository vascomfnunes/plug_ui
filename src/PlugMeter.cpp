...
void PlugMeter::paint(juce::Graphics& g) {
    ...
    auto meterFont = juce::Font(juce::FontOptions(fontFamily, 11.5f, juce::Font::bold));
    ...
    g.drawText("GR", inner.withY(barY).withHeight(barH).withWidth(labelW),
               fullyJustify ? juce::Justification::centredLeft : juce::Justification::centred);
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        if (norm > 0.001f && segCenterX < barLeft + fillW) {
            ...
            float alpha = 1.0f;
            if (norm < 1.0f) {
                const float segNorm = static_cast<float>(i) / static_cast<float>(segments);
                if (segNorm > fadeStart) {
                    alpha =
                        juce::jlimit(0.0f, 1.0f, 1.0f - (segNorm - fadeStart) / (norm - fadeStart));
                    alpha = alpha * alpha; // ease-out curve
                }
            }
            const float customContrast = getProperties().getWithDefault("barContrast", 1.0f);
    ...
    for (const Tick tick :
         {Tick{0.0f, "0"}, Tick{5.0f, "-5"}, Tick{10.0f, "-10"}, Tick{15.0f, "-15"}}) {
        const float x = barLeft + barMaxW * juce::jlimit(0.0f, 1.0f, tick.db / m_maxGrDb);
        g.drawText(tick.text, juce::Rectangle<float>(x - 20.0f, barY + barH + 2.0f, 40.0f, 10.0f),
                   juce::Justification::centred, false);
    }

    auto valFont = juce::Font(juce::FontOptions(fontFamily, 16.5f * fontScale, juce::Font::bold));
    ...
    if (fullyJustify) {
        g.drawText(dbText, inner.withLeft(barRight).withWidth(dbTextW).withY(barY).withHeight(barH),
                   juce::Justification::centredRight);
    } else {
        g.drawText(dbText, inner.withLeft(barRight + 8.0f).withY(barY).withHeight(barH),
