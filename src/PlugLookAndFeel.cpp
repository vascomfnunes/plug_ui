...
void PlugLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height,
    ...
    // ── Soft drop shadow (radial gradient, fades well before edge) ───────────
    {
        ...
        juce::ColourGradient shadowGrad(juce::Colours::black.withAlpha(0.56f), centre.x,
                                        centre.y + radius * 0.11f, juce::Colours::transparentBlack,
                                        centre.x + shadowR, centre.y + radius * 0.11f, true);
    ...
    // ── Wide dark rim / shelf / bezel ────────────────────────────────────────
    {
        juce::ColourGradient rimGrad(knobRimLight.brighter(0.08f), centre.x, knobBounds.getY(),
                                     knobRimDark.darker(0.10f), centre.x, knobBounds.getBottom(),
                                     false);
    ...
    // ── Knob body ────────────────────────────────────────────────────────────
    {
        ...
        juce::ColourGradient bodyGrad(knobFillCentre.brighter(0.12f), centre.x, bodyBounds.getY(),
                                      knobFill.darker(0.30f), centre.x, bodyBounds.getBottom(),
                                      false);
    ...
    // ── White pointer / indicator line ───────────────────────────────────────
    {
        ...
        juce::Path indicator;
        indicator.addRoundedRectangle(-pointerThickness * 0.5f, -pointerLength, pointerThickness,
                                      pointerLength, pointerThickness * 0.35f);
...
void PlugLookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height,
...
void PlugLookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& button,
    ...
    auto paintLedGlow = [&g](juce::Rectangle<float> bounds, float corner, juce::Colour ledBase,
                             float haloAlpha, float innerAlpha) {
        ...
        juce::ColourGradient inner(juce::Colours::white.withAlpha(innerAlpha), bounds.getCentreX(),
                                   bounds.getY() + 1.2f, juce::Colours::transparentWhite,
    ...
    auto paintUnlitCap = [&g](juce::Rectangle<float> bounds, float corner) {
        ...
        juce::ColourGradient innerDim(juce::Colours::transparentBlack, bounds.getCentreX(),
                                      bounds.getY() + 1.0f, juce::Colours::black.withAlpha(0.12f),
                                      bounds.getCentreX(), bounds.getBottom() - 1.0f, false);
    ...
    if (isSegment) {
        ...
        juce::ColourGradient fill(base.brighter(isOn ? 0.16f : 0.10f), r.getCentreX(), r.getY(),
                                  base.darker(isOn ? 0.18f : 0.14f), r.getCentreX(), r.getBottom(),
                                  false);
        ...
        g.drawLine(r.getX() + 1.0f, r.getBottom() - 1.0f, r.getRight() - 1.0f, r.getBottom() - 1.0f,
                   0.9f);
        ...
        auto btnFont = juce::Font(
            juce::FontOptions(fontFamily, juce::jmax(fontSegment, b.getHeight() * 0.42f) * fontScale,
                              juce::Font::plain));
        ...
        const juce::Colour activeBase =
            button.getProperties().contains("plugPillRed") ? juce::Colour(0xffca5547) : accent;
        ...
        g.drawLine(r.getX() + 1.2f, r.getBottom() - 1.0f, r.getRight() - 1.2f, r.getBottom() - 1.0f,
                   0.9f);
        ...
        juce::ColourGradient fill(bgColour.brighter(hoverBoost), r.getCentreX(), r.getY(),
                                  bgColour.darker(0.16f), r.getCentreX(), r.getBottom(), false);
        g.setGradientFill(fill);
...
void PlugLookAndFeel::drawButtonText(juce::Graphics& g, juce::TextButton& button, bool, bool) {
    const auto b = button.getLocalBounds().toFloat();
    auto f       = juce::Font(
        juce::FontOptions(fontFamily, juce::jmax(8.0f, b.getHeight() * 0.42f), juce::Font::plain));
...
void PlugLookAndFeel::drawComboBox(juce::Graphics& g, int width, int height, bool, int buttonX,
    ...
    const bool emphasizeCharacter =
        box.getProperties().getWithDefault("plugCharacterEmphasis", false);
    ...
    juce::ColourGradient fill(panelBg.brighter(topTone), bounds.getCentreX(), bounds.getY(),
                              panelBg.darker(bottomTone), bounds.getCentreX(), bounds.getBottom(),
                              false);
    g.setGradientFill(fill);
    g.fillRoundedRectangle(bounds, 2.0f);
    ...
    if (emphasizeCharacter) {
        const bool active =
            box.hasKeyboardFocus(true) || box.isMouseOverOrDragging() || box.getSelectedId() > 0;
...
juce::Font PlugLookAndFeel::getComboBoxFont(juce::ComboBox& box) {
    const bool emphasizeCharacter =
        box.getProperties().getWithDefault("plugCharacterEmphasis", false);
    auto f = juce::Font(
        juce::FontOptions(fontFamily, emphasizeCharacter ? 11.9f : 11.0f, juce::Font::plain));
