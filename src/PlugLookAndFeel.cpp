...
void PlugLookAndFeel::drawButtonBackground(juce::Graphics &g, juce::Button &button,
  ...
  const bool isPrimary =
      button.getProperties().getWithDefault("plugPrimaryAction", false);

  if (isPrimary) {
    auto shadowBounds = bounds.translated(0.0f, shouldDrawButtonAsDown ? 0.4f : 1.1f)
                            .expanded(1.4f, 1.1f);
  ...
  juce::ColourGradient fill(base.brighter(topBoost), bounds.getCentreX(), bounds.getY(),
                            base.darker(bottomDark), bounds.getCentreX(),
