...
juce::Font PlugLookAndFeel::getLabelFont(juce::Label &label) {
  ...
  if (f.getHeight() > 1.0f)
    return f;
  auto small =
      juce::Font(juce::FontOptions(fontFamily, fontValue, juce::Font::plain));
...
// ========================================================
void PlugLookAndFeel::drawRotarySlider(juce::Graphics &g, int x, int y,
                                       int width, int height, float sliderPos,
                                       float rotaryStartAngle,
                                       float rotaryEndAngle,
                                       juce::Slider &slider) {
  auto fullBounds = juce::Rectangle<float>(
  ...
  // ── Soft drop shadow (radial gradient, fades well before edge) ───────────
  {
    ...
    juce::ColourGradient shadowGrad(
        juce::Colours::black.withAlpha(0.56f), centre.x,
  ...
  // ── Wide dark rim / shelf / bezel ────────────────────────────────────────
  {
    ...
    // Faint top-edge highlight
    {
      ...
      topArc.addCentredArc(centre.x, centre.y, radius - 0.5f, radius - 0.5f,
                           0.0f, -juce::MathConstants<float>::pi * 0.85f,
  ...
  // ── White pointer / indicator line ───────────────────────────────────────
  {
    ...
    g.fillPath(indicator, juce::AffineTransform::rotation(angle).translated(
                              centre.x, centre.y));
  }
...
void PlugLookAndFeel::drawLinearSlider(juce::Graphics &g, int x, int y,
                                       int width, int height, float sliderPos,
  ...
  g.fillRoundedRectangle(left, midY - trackThick * 0.5f,
                         juce::jmax(0.0f, right - left), trackThick,
                         trackThick * 0.5f);
  ...
  g.fillEllipse(sliderPos - thumbR, midY - thumbR, thumbR * 2.0f,
                thumbR * 2.0f);
  g.setColour(accentBright);
  g.drawEllipse(sliderPos - thumbR, midY - thumbR, thumbR * 2.0f, thumbR * 2.0f,
                1.0f);
}
...
// Layout or wiring changes should happen elsewhere.
// ========================================================
void PlugLookAndFeel::drawToggleButton(juce::Graphics &g,
  ...
  auto paintLedGlow = [&g](juce::Rectangle<float> bounds, float corner,
                           juce::Colour ledBase, float haloAlpha,
                           float innerAlpha) {
    ...
    juce::ColourGradient halo(
        ledBase.withAlpha(haloAlpha), glowBounds.getCentreX(),
    ...
    juce::ColourGradient inner(
        juce::Colours::white.withAlpha(innerAlpha), bounds.getCentreX(),
        bounds.getY() + 1.2f, juce::Colours::transparentWhite,
        bounds.getCentreX(), bounds.getBottom() - 1.0f, false);
    g.setGradientFill(inner);
    g.fillRoundedRectangle(bounds.reduced(1.0f),
                           juce::jmax(1.2f, corner - 1.0f));
  };
  ...
  if (isSegment) {
    ...
    juce::ColourGradient fill(base.brighter(isOn ? 0.16f : 0.10f),
                              r.getCentreX(), r.getY(),
                              base.darker(isOn ? 0.18f : 0.14f), r.getCentreX(),
                              r.getBottom(), false);
    ...
    g.drawLine(r.getX() + 1.2f, r.getY() + 1.0f, r.getRight() - 1.2f,
               r.getY() + 1.0f, 0.8f);
    ...
    const juce::Colour activeBase =
        button.getProperties().contains("plugPillRed")
    ...
    juce::ColourGradient fill(base.brighter(isOn ? 0.18f : 0.10f),
                              r.getCentreX(), r.getY() + 0.5f,
                              base.darker(isOn ? 0.22f : 0.16f), r.getCentreX(),
                              r.getBottom() - 0.5f, false);
    g.setGradientFill(fill);
    g.fillRoundedRectangle(r, corner);
    ...
    auto btnFont =
        juce::Font(juce::FontOptions(fontFamily, fontSize, juce::Font::plain));
    ...
    juce::ColourGradient fill(bgColour.brighter(hoverBoost), r.getCentreX(),
                              r.getY(), bgColour.darker(0.16f), r.getCentreX(),
                              r.getBottom(), false);
    g.setGradientFill(fill);
    g.fillRoundedRectangle(r, cornerRadius);
    g.setColour(juce::Colours::white.withAlpha(isOn ? 0.20f : 0.10f));
    g.drawLine(r.getX() + 1.0f, r.getY() + 1.0f, r.getRight() - 1.0f,
               r.getY() + 1.0f, 0.8f);
    g.setColour(juce::Colours::black.withAlpha(0.22f));
    g.drawLine(r.getX() + 1.0f, r.getBottom() - 1.0f, r.getRight() - 1.0f,
               r.getBottom() - 1.0f, 0.9f);
    ...
    auto btnFont =
        juce::Font(juce::FontOptions(fontFamily, 8.5f, juce::Font::plain));
...
void PlugLookAndFeel::drawButtonBackground(juce::Graphics &g,
                                           juce::Button &button,
  ...
  g.drawLine(bounds.getX() + 1.0f, bounds.getY() + 1.0f,
             bounds.getRight() - 1.0f, bounds.getY() + 1.0f, 0.8f);
  g.setColour(juce::Colours::black.withAlpha(0.25f));
  g.drawLine(bounds.getX() + 1.0f, bounds.getBottom() - 1.0f,
             bounds.getRight() - 1.0f, bounds.getBottom() - 1.0f, 0.8f);
  g.setColour(borderSubtle.withAlpha(0.68f));
  g.drawRoundedRectangle(bounds, 2.0f, 0.8f);
}

void PlugLookAndFeel::drawButtonText(juce::Graphics &g,
...
void PlugLookAndFeel::drawLabel(juce::Graphics &g, juce::Label &label) {
  ...
  auto font = juce::Font(juce::FontOptions(
      fontFamily, label.getFont().getHeight(), juce::Font::plain));
...
void PlugLookAndFeel::positionComboBoxText(juce::ComboBox &box,
                                           juce::Label &label) {
