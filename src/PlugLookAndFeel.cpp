...
int PlugLookAndFeel::getSliderPopupPlacement (juce::Slider&)
{
    ...
    return juce::BubbleComponent::above | juce::BubbleComponent::below | juce::BubbleComponent::left
           | juce::BubbleComponent::right;
}

void PlugLookAndFeel::drawBubble (juce::Graphics& g, juce::BubbleComponent&, const juce::Point<float>& tip,
                                  const juce::Rectangle<float>& body)
{
    // Near-black, slightly warm, semi-transparent — matches the dark glass
    // aesthetic of the meter and knob faces.
    juce::Path p;
    p.addBubble (body.reduced (0.5f), body.getUnion (juce::Rectangle<float> (tip.x, tip.y, 1.0f, 1.0f)), tip, 4.0f,
                 5.5f);
...
// ========================================================
void PlugLookAndFeel::drawRotarySlider (juce::Graphics& g, int x, int y, int width, int height, float sliderPos,
                                        float rotaryStartAngle, float rotaryEndAngle, juce::Slider& slider)
{
    ...
    // ── Soft drop shadow (radial gradient, fades well before edge) ───────────
    {
        ...
        auto shadowBounds =
            fullBounds.withSizeKeepingCentre (shadowR * 2.0f, shadowR * 2.0f).translated (0.0f, radius * 0.11f);
        juce::ColourGradient shadowGrad (juce::Colours::black.withAlpha (0.56f), centre.x, centre.y + radius * 0.11f,
                                         juce::Colours::transparentBlack, centre.x + shadowR, centre.y + radius * 0.11f,
                                         true);
    ...
    {
        juce::ColourGradient rimGrad (knobRimLight.brighter (0.08f), centre.x, knobBounds.getY(),
                                      knobRimDark.darker (0.10f), centre.x, knobBounds.getBottom(), false);
...

        {
            ...
            topArc.addCentredArc (centre.x, centre.y, radius - 0.5f, radius - 0.5f, 0.0f,
                                  -juce::MathConstants<float>::pi * 0.85f, juce::MathConstants<float>::pi * 0.85f,
                                  true);
    ...
    // ── Directional shadow on bezel (bottom-right) ────────────────────────────
    {
        ...
        juce::ColourGradient dirShadow (juce::Colours::transparentBlack, centre.x + shadowOff * 0.3f,
    ...
    {
        const float bodyR = bodyBounds.getWidth() * 0.5f;
        juce::ColourGradient bodyGrad (knobFillCentre.brighter (0.12f), centre.x, bodyBounds.getY(),
                                       knobFill.darker (0.30f), centre.x, bodyBounds.getBottom(), false);
        ...
        juce::ColourGradient lowerShadow (juce::Colours::transparentBlack, centre.x, centre.y,
                                          juce::Colours::black.withAlpha (0.42f), centre.x, bodyBounds.getBottom(),
                                          false);
        ...
        specular.addCentredArc (centre.x, centre.y, bodyR * 0.62f, bodyR * 0.62f, 0.0f,
                                -juce::MathConstants<float>::pi * 0.55f, juce::MathConstants<float>::pi * 0.55f, true);
        juce::ColourGradient specGrad (juce::Colours::white.withAlpha (0.16f), centre.x,
                                       bodyBounds.getY() + bodyR * 0.22f, juce::Colours::transparentWhite, centre.x,
    ...
    // ── White pointer / indicator line ───────────────────────────────────────
    {
        ...
        juce::Path indicator;
        indicator.addRoundedRectangle (-pointerThickness * 0.5f, -pointerLength, pointerThickness, pointerLength,
...
void PlugLookAndFeel::drawLinearSlider (juce::Graphics& g, int x, int y, int width, int height, float sliderPos, float,
                                        float, juce::Slider::SliderStyle, juce::Slider&)
{
    ...
    g.fillRoundedRectangle (left, midY - trackThick * 0.5f, juce::jmax (0.0f, right - left), trackThick,
                            trackThick * 0.5f);

    const float fillW = juce::jmax (0.0f, sliderPos - left);
    g.setColour (accentBright.withAlpha (0.92f));
    g.fillRoundedRectangle (left, midY - trackThick * 0.5f, fillW, trackThick, trackThick * 0.5f);
                                        ...
                                        bool shouldDrawButtonAsHighlighted, bool /*shouldDrawButtonAsDown*/)
{
    ...
    auto paintLedGlow =
        [&g] (juce::Rectangle<float> bounds, float corner, juce::Colour ledBase, float haloAlpha, float innerAlpha)
    {
        // Outer soft bloom to simulate light bleeding from a lit cap.
        auto glowBounds = bounds.expanded (3.0f, 2.0f);
        juce::ColourGradient halo (ledBase.withAlpha (haloAlpha), glowBounds.getCentreX(), glowBounds.getCentreY(),
                                   juce::Colours::transparentBlack, glowBounds.getRight(), glowBounds.getBottom(),
                                   true);
        ...
        juce::ColourGradient inner (juce::Colours::white.withAlpha (innerAlpha), bounds.getCentreX(),
                                    bounds.getY() + 1.2f, juce::Colours::transparentWhite, bounds.getCentreX(),
    ...
    auto paintUnlitCap = [&g] (juce::Rectangle<float> bounds, float corner)
    {
        ...
        juce::ColourGradient smoke (juce::Colour (0xff000000).withAlpha (0.10f), bounds.getCentreX(),
                                    bounds.getY() + 1.0f, juce::Colour (0xff000000).withAlpha (0.20f),
        ...
        juce::ColourGradient innerDim (juce::Colours::transparentBlack, bounds.getCentreX(), bounds.getY() + 1.0f,
                                       juce::Colours::black.withAlpha (0.12f), bounds.getCentreX(),
                                       bounds.getBottom() - 1.0f, false);
    ...
    if (isSegment)
    {
        ...
        juce::ColourGradient fill (base.brighter (isOn ? 0.16f : 0.10f), r.getCentreX(), r.getY(),
                                   base.darker (isOn ? 0.18f : 0.14f), r.getCentreX(), r.getBottom(), false);
        g.setGradientFill (fill);
        g.fillRoundedRectangle (r, corner);

        // Bevel cues: fine top light and lower shade.
        g.setColour (juce::Colours::white.withAlpha (isOn ? 0.22f : 0.12f));
        g.drawLine (r.getX() + 1.2f, r.getY() + 1.0f, r.getRight() - 1.2f, r.getY() + 1.0f, 0.8f);
        g.setColour (juce::Colours::black.withAlpha (isOn ? 0.18f : 0.28f));
        g.drawLine (r.getX() + 1.0f, r.getBottom() - 1.0f, r.getRight() - 1.0f, r.getBottom() - 1.0f, 0.9f);
        ...
        auto btnFont = juce::Font (juce::FontOptions (fontFamily, juce::jmax (fontSegment, b.getHeight() * 0.42f),
                                                      juce::Font::plain));
        btnFont.setExtraKerningFactor (0.02f);
        g.setFont (btnFont);
        g.setColour (isOn ? buttonOnText : buttonOffText);
        g.drawFittedText (button.getButtonText().toUpperCase(), b.toNearestInt(), juce::Justification::centred, 1);
    }
    else if (isPill)
    {
        // ── Pill button (A, LF, etc.) ────────────────────────────────────────
        const float corner = b.getHeight() * 0.28f;
        const juce::Colour activeBase = button.getProperties().contains ("plugPillRed") ? juce::Colour (0xffca5547)
                                                                                        : accent;
        ...
        juce::ColourGradient fill (base.brighter (isOn ? 0.18f : 0.10f), r.getCentreX(), r.getY() + 0.5f,
                                   base.darker (isOn ? 0.22f : 0.16f), r.getCentreX(), r.getBottom() - 0.5f, false);
        g.setGradientFill (fill);
        g.fillRoundedRectangle (r, corner);

        // Subtle inset at bottom to increase "pressed hardware cap" feel.
        g.setColour (juce::Colours::black.withAlpha (isOn ? 0.20f : 0.30f));
        g.drawLine (r.getX() + 1.2f, r.getBottom() - 1.0f, r.getRight() - 1.2f, r.getBottom() - 1.0f, 0.9f);
        g.setColour (juce::Colours::white.withAlpha (isOn ? 0.24f : 0.12f));
        g.drawLine (r.getX() + 1.2f, r.getY() + 1.0f, r.getRight() - 1.2f, r.getY() + 1.0f, 0.8f);
        g.setColour (borderSubtle.withAlpha (isOn ? 0.48f : 0.70f));
        g.drawRoundedRectangle (r, corner, 0.8f);

        const float fontSize = juce::jmax (7.0f, b.getHeight() * 0.56f);
    ...
    else
    {
        ...
        juce::ColourGradient fill (bgColour.brighter (hoverBoost), r.getCentreX(), r.getY(), bgColour.darker (0.16f),
        ...
        g.drawLine (r.getX() + 1.0f, r.getY() + 1.0f, r.getRight() - 1.0f, r.getY() + 1.0f, 0.8f);
        g.setColour (juce::Colours::black.withAlpha (0.22f));
        g.drawLine (r.getX() + 1.0f, r.getBottom() - 1.0f, r.getRight() - 1.0f, r.getBottom() - 1.0f, 0.9f);
                                            ...
                                            bool shouldDrawButtonAsDown)
{
    ...
    if (isPrimary)
    {
        auto shadowBounds = bounds.translated (0.0f, shouldDrawButtonAsDown ? 0.4f : 1.1f).expanded (1.4f, 1.1f);
        juce::ColourGradient shadow (juce::Colours::black.withAlpha (0.24f), shadowBounds.getCentreX(),
                                     shadowBounds.getCentreY(), juce::Colours::transparentBlack,
                                     shadowBounds.getRight(), shadowBounds.getBottom(), true);
    ...
    juce::ColourGradient fill (base.brighter (topBoost), bounds.getCentreX(), bounds.getY(), base.darker (bottomDark),
                               bounds.getCentreX(), bounds.getBottom(), false);
    g.setGradientFill (fill);
    ...
    if (isPrimary && shouldDrawButtonAsHighlighted)
    {
        ...
        juce::ColourGradient glow (accentBright.withAlpha (0.20f), glowBounds.getCentreX(), glowBounds.getCentreY(),
                                   juce::Colours::transparentBlack, glowBounds.getRight(), glowBounds.getBottom(),
                                   true);
    ...
    g.drawLine (bounds.getX() + 1.0f, bounds.getY() + 1.0f, bounds.getRight() - 1.0f, bounds.getY() + 1.0f, 0.8f);
    g.setColour (juce::Colours::black.withAlpha (0.25f));
    g.drawLine (bounds.getX() + 1.0f, bounds.getBottom() - 1.0f, bounds.getRight() - 1.0f, bounds.getBottom() - 1.0f,
                0.8f);
    g.setColour (borderSubtle.withAlpha (0.68f));
    g.drawRoundedRectangle (bounds, 2.0f, 0.8f);
}

void PlugLookAndFeel::drawButtonText (juce::Graphics& g, juce::TextButton& button, bool, bool)
{
    const auto b = button.getLocalBounds().toFloat();
    auto f = juce::Font (juce::FontOptions (fontFamily, juce::jmax (8.0f, b.getHeight() * 0.42f), juce::Font::plain));
...
void PlugLookAndFeel::drawLabel (juce::Graphics& g, juce::Label& label)
{
    ...
    auto font = juce::Font (juce::FontOptions (fontFamily, label.getFont().getHeight(), juce::Font::plain));
...
// Layout or wiring changes should happen elsewhere.
// ========================================================
void PlugLookAndFeel::drawComboBox (juce::Graphics& g, int width, int height, bool, int buttonX, int buttonY,
                                    int buttonW, int buttonH, juce::ComboBox& box)
{
    ...
    juce::ColourGradient fill (panelBg.brighter (topTone), bounds.getCentreX(), bounds.getY(),
                               panelBg.darker (bottomTone), bounds.getCentreX(), bounds.getBottom(), false);
    g.setGradientFill (fill);
    g.fillRoundedRectangle (bounds, 2.0f);
    ...
    {
        const bool active = box.hasKeyboardFocus (true) || box.isMouseOverOrDragging() || box.getSelectedId() > 0;
        g.setColour (accentBright.withAlpha (active ? 0.55f : 0.24f));
        g.drawLine (bounds.getX() + 6.0f, bounds.getBottom() - 1.0f, bounds.getRight() - 6.0f,
                    bounds.getBottom() - 1.0f, 1.4f);
    }
}
...
{
    const bool emphasizeCharacter = box.getProperties().getWithDefault ("plugCharacterEmphasis", false);
    auto f = juce::Font (juce::FontOptions (fontFamily, emphasizeCharacter ? 11.9f : 11.0f, juce::Font::plain));
...
void PlugLookAndFeel::positionComboBoxText (juce::ComboBox& box, juce::Label& label)
{
    ...
    label.setBounds (leftPadding, verticalPadding, juce::jmax (0, box.getWidth() - leftPadding - rightPadding),
