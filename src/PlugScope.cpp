...
void PlugScope::pushColumn(float preMin, float preMax, float postMin, float postMax) {
...
void PlugScope::paint(juce::Graphics& g) {
    ...
    // ── Clip-zone band fill ─────────────────────────────────────────────────
    {
        // Positive half
        auto clipZone =
            juce::Rectangle<float>(bounds.getX(), clipY, bounds.getWidth(), threshY - clipY);
        ...
        auto clipZoneNeg = juce::Rectangle<float>(bounds.getX(), negThreshY, bounds.getWidth(),
                                                  negClipY - negThreshY);
    ...
    // 1) Pre-clip ghost outline
    {
        ...
        if (!preOutline.isEmpty()) {
            ...
            g.strokePath(preOutline, juce::PathStrokeType(1.0f, juce::PathStrokeType::curved,
    ...
    // 2) Post-clip filled body
    {
        ...
        if (!postFill.isEmpty()) {
            ...
            g.strokePath(postFill, juce::PathStrokeType(1.0f, juce::PathStrokeType::curved,
                                                        juce::PathStrokeType::rounded));
        }
    }
    ...
    // ── Single clip ceiling line ────────────────────────────────────────────
    {
        ...
        g.drawLine(waveBounds.getX(), negClipY, waveBounds.getRight(), negClipY, 0.5f);
    }

    // ── Label: clip ceiling dB value ────────────────────────────────────────
    {
        auto labelFont = juce::Font(juce::FontOptions(Theme::fontFamily, 8.0f, juce::Font::plain));
        ...
        auto labelRect = juce::Rectangle<float>(labelX, labelY, labelW, labelH).expanded(1.0f);
        ...
        g.drawText(labelStr, static_cast<int>(labelX), static_cast<int>(labelY),
                   static_cast<int>(labelW), static_cast<int>(labelH), juce::Justification::centred,
                   false);
    }
