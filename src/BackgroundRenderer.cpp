...
// ========================================================
void BackgroundRenderer::paintMainBackground(juce::Graphics &g,
  ...
  juce::ColourGradient lift(juce::Colour(0xFF181E29).withAlpha(0.25f), cx,
                            cy * 0.9f, juce::Colour(0xFF181E29).withAlpha(0.0f),
                            cx, h * 0.9f, true);
  ...
  if (textureToDraw != nullptr && textureToDraw->isValid()) {
    ...
    if (clampedOpacity > 0.0f) {
      ...
      g.drawImageWithin(*textureToDraw, bounds.getX(), bounds.getY(),
...
// Layout or wiring changes should happen elsewhere.
// ========================================================
void BackgroundRenderer::paintEnginePanel(juce::Graphics &g,
                                          juce::Rectangle<int> bounds) {
  ...
  juce::ColourGradient base(juce::Colour(0xFF000000).withAlpha(0.06f),
                            inner.getCentreX(), inner.getY(),
                            juce::Colour(0xFF000000).withAlpha(0.13f),
  ...
  g.drawLine(topEdge.getX() + 9.0f, topEdge.getBottom() - 0.9f,
             topEdge.getRight() - 9.0f, topEdge.getBottom() - 0.9f, 1.0f);

  // Local floor shadow to seat controls in the panel cavity.
  juce::ColourGradient floorShadow(
      juce::Colours::transparentBlack, inner.getCentreX(), inner.getCentreY(),
      juce::Colours::black.withAlpha(0.16f), inner.getCentreX(),
      inner.getBottom() + 22.0f, false);
