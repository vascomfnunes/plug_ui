...
// Major ticks: 0, 2, 4, 6, 8, 10, 15 dB.
//
...
void PlugGrMeter::paint(juce::Graphics &g) {
  ...
  const auto centre = juce::Point<float>(
      std::floor(faceBounds.getCentreX()) + 0.5f,
      std::floor(faceBounds.getBottom() - 11.0f * s) + 0.5f);
  const float radius =
      juce::jmin(faceBounds.getWidth() * 0.47f, faceBounds.getHeight() * 0.90f) - 4.0f * s;

  // ── Ticks ─────────────────────────────────────────────────────────────────
  const std::array<float, 7> majorTicks   = {0.0f, 2.0f, 4.0f, 6.0f, 8.0f, 10.0f, 15.0f};
  const std::array<float, 7> labeledTicks = {0.0f, 2.0f, 4.0f, 6.0f, 8.0f, 10.0f, 15.0f};

  g.setFont(juce::Font(juce::FontOptions(fontFamily, 8.8f * s, juce::Font::bold)));

  for (float gr = 0.0f; gr <= maxGr + 0.5f; gr += 1.0f) {
    bool isMajor = false;
    for (float t : majorTicks)
      if (std::abs(gr - t) < 0.1f) { isMajor = true; break; }
    ...
    const auto tickCol = isMajor
                             ? amberMid.withAlpha(0.90f)
                             : amberDim.withAlpha(0.65f);
    g.setColour(tickCol);
    ...
    for (float l : labeledTicks)
      if (std::abs(gr - l) < 0.1f) { showLabel = true; break; }
    if (!showLabel) continue;

    const juce::String labelText = (gr < 0.1f)
        ? juce::String("0")
        : ("-" + juce::String(static_cast<int>(gr)));
    ...
    if (gr < 0.1f) {
      tangentOffset += 2.0f * s;  radialOffset += 1.0f * s;
    } else if (gr > 1.9f && gr < 2.1f) {
      tangentOffset += 1.2f * s;  radialOffset -= 1.4f * s;
    } else if (gr > 3.9f && gr < 4.1f) {
      tangentOffset += 0.5f * s;  radialOffset -= 1.6f * s;
    } else if (gr > 5.9f && gr < 6.1f) {
      tangentOffset -= 0.2f * s;  radialOffset -= 1.6f * s;
    } else if (gr > 7.9f && gr < 8.1f) {
      tangentOffset -= 1.2f * s;  radialOffset -= 1.4f * s;
    } else if (gr > 9.9f && gr < 10.1f) {
      tangentOffset -= 1.2f * s;  radialOffset -= 1.0f * s;
    } else if (gr > 14.9f) {
      tangentOffset -= 2.0f * s;  radialOffset += 1.0f * s;
    }
    ...
    const auto labelPos  = juce::Point<float>(
        centre.x + radialX * labelR + tangentX * tangentOffset,
    ...
    g.drawText(labelText,
               static_cast<int>(labelPos.x - 12.0f * s),
               static_cast<int>(labelPos.y - 5.5f * s),
               lW, lH, juce::Justification::centred);
  }

  // ── Digital GR readout ────────────────────────────────────────────────────
  {
    const juce::String grText = (m_grDb > 0.05f)
                                    ? "-" + juce::String(m_grDb, 1) + " dB"
                                    : "0.0 dB";
    ...
    g.drawText(grText,
               static_cast<int>(faceBounds.getX()),
               static_cast<int>(centre.y - 31.0f * s),
               static_cast<int>(faceBounds.getWidth()),
               static_cast<int>(16.0f * s),
    ...
    g.drawText("GR",
               static_cast<int>(faceBounds.getX()),
               static_cast<int>(centre.y - 16.0f * s),
               static_cast<int>(faceBounds.getWidth()),
               static_cast<int>(10.0f * s),
               juce::Justification::centred);
  }
  ...
  if (clampedGr > 0.1f) {
    for (int layer = 3; layer >= 1; --layer) {
      ...
      g.strokePath(glow,
                   juce::PathStrokeType(static_cast<float>(layer) * 3.0f * s,
