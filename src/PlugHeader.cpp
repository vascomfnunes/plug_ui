...
PlugHeader::PlugHeader(const juce::String &title,
                       const juce::String & /*version*/,
                       const juce::String &subtitle)
    : m_title(title), m_subtitle(subtitle) {}

void PlugHeader::setUiScale(float scale) { m_uiScale = scale; }

void PlugHeader::setColRatio(float ratio) { m_colRatio = ratio; }
...
void PlugHeader::paint(juce::Graphics &g) {
  ...
  const int colW =
      juce::roundToInt(static_cast<float>(header.getWidth()) * colRatio);
  auto leftCol = header.withWidth(colW);
  auto rightCol = header.withLeft(header.getRight() - colW)
                      .withWidth(colW)
                      .translated(0, 2);
  ...
  auto subtitleFont = juce::Font(
      juce::FontOptions(fontFamily, 20.0f * m_uiScale, juce::Font::plain));
